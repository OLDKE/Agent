...
//	char tmpbuf[128];
	//这个m_trace就是一个跟踪日志的对象，主要用来记录日志
	m_trace= new CNTTRACE;
	char LogCFG[256];
	//从注册表中读取日志文件的配置文件路径，（此路径不会自动创建，需要手动创建！ 坑）
	...
	//WTrace用来记录日志， 第一个参数日志配置文件中的模块小节名， 第二各参数日志文件的名字， 第三个参数日志的级别， 第四个参数 记录的内容
...
BOOL CNTAgent3Dlg::OnInitDialog()
{
	...
	//开启agent各个模块， 交易，发送，接收
 	m_ntlogic->StartSer("Trn");
 	m_ntlogic->StartSer("Snd");	
 	//m_ntlogic->StartSer("RtC");
	m_ntlogic->StartSer("Rev");

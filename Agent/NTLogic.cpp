...
BEGIN_MESSAGE_MAP(NTLogic, CDialog)
	ON_WM_TIMER()
	ON_MESSAGE(WM_UDPRecv, OnRecvLogic)
...
LRESULT NTLogic::OnRecvLogic(WPARAM wParam, LPARAM lParam)
{
	...
	ret = m_packxml->UnPack(recvbuf, strlen(recvbuf), &vmsg);
	...
	if( -6 == ret )
	{
		...
		m_packxml->PackComdRet(sendmsg, vmsg.TRNCODE, RET_FAIL, vmsg.TRNEJ, "设备号校验错");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if( !strcmp("60001", vmsg.TRNCODE) )		//更新配置
	{
// 		memset(trncode,0,sizeof(trncode));
// 		GetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE",trncode);
// 		SetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE_P_TMP",trncode);
// 		memset(trnej_tmp,0,sizeof(trnej_tmp));
// 		GetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNEJ",trnej_tmp);
// 		SetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNEJ_P_TMP",trnej_tmp);
// 
// 		memset(sendmsg,0,sizeof(sendmsg));
// 		PackComdRet(sendmsg,trncode,"3",0,"");
// 		m_udpser->SendMsg(sendmsg,strlen(sendmsg));
	}
	else if( !strcmp("60002", vmsg.TRNCODE) )		//启动应用
	{
	}
	else if( !strcmp("60003", vmsg.TRNCODE) )		//暂停服务
	{	
	}
	else if( !strcmp("60004", vmsg.TRNCODE) )		//重启设备
	{
	}
	else if( !strcmp("60005", vmsg.TRNCODE) )		//强制重启
	{
	}
	else if( !strcmp("60006", vmsg.TRNCODE) )		//设备关机
	{
	}
	else if( !strcmp("60007", vmsg.TRNCODE) )		//强制关机
	{
	}
	else if( !strcmp("60008", vmsg.TRNCODE) )		//更新时间
	{
	}
	else if( !strcmp("60009", vmsg.TRNCODE) )		//上传文件
	{
// 		memset(trncode,0,sizeof(trncode));
// 		GetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE",trncode);
// 		SetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE_P_TMP",trncode);
...
// 
// 		memset(sendmsg,0,sizeof(sendmsg));
// 		PackComdRet(sendmsg,trncode,"2",0,"");
// 		m_udpser->SendMsg(sendmsg,strlen(sendmsg));
// 
// 		SetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","CMD_DELAY","1");
// 		s_ThreadPara *m_threadpara = new  s_ThreadPara;
// 		m_threadpara->m_ntlogic=this;
// 		strcpy(m_threadpara->trncode,trncode);
// 		m_cmdrun = CreateThread(NULL,0,CmdRUN,(LPVOID)m_threadpara,0,NULL);
// 		if(m_cmdrun==NULL)
// 		{
// 			m_trace->WTrace(LOG_GRP,LOG_STEP,LT_INFO,"CreateThread CmdRUN error!!!");
// 			return 0;
// 		}		
	}
	else if( !strcmp("60010", vmsg.TRNCODE) )		//软件更新
	{
// 		memset(trncode,0,sizeof(trncode));
// 		GetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE",trncode);
// 		SetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE_P_TMP",trncode);
...
// 			memset(sendmsg,0,sizeof(sendmsg));
// 			PackComdRet(sendmsg,trncode,"3",0,"");
// 			m_trace->WTrace(LOG_GRP,LOG_STEP,LT_INFO,"不用更新！！");
// 			m_udpser->SendMsg(sendmsg,strlen(sendmsg));
// 		}
	}
	else if( !strcmp("60011", vmsg.TRNCODE) )		//强制软件更新
	{
// 		memset(trncode,0,sizeof(trncode));
// 		GetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE",trncode);
// 		SetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE_P_TMP",trncode);
...
// 		}		
	...
	else if( !strcmp("60012", vmsg.TRNCODE) )		//广告更新
	{
// 		memset(trncode,0,sizeof(trncode));
// 		GetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE",trncode);
// 		SetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE_P_TMP",trncode);
...
// 		}
	...
	else if( !strcmp("60013", vmsg.TRNCODE) )		//强制广告更新
	{
// 		memset(trncode,0,sizeof(trncode));
// 		GetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE",trncode);
// 		SetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE_P_TMP",trncode);
...
// 		}		
	...
	else if( !strcmp("60014", vmsg.TRNCODE) )  //上传文件明细
	{
		
	}
	else if( !strcmp("60018", vmsg.TRNCODE) )  //Agent升级
	{
// 		memset(trncode,0,sizeof(trncode));
// 		GetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE",trncode);
// 		SetNTReg("SOFTWARE\\Ebring\\Agent\\TMP","TRNCODE_P_TMP",trncode);
...
// 		}		
	...
	else if( !strcmp("60019", vmsg.TRNCODE) )  //上传系统运行状态
	{
		
	}
	else if ( !strcmp("60023", vmsg.TRNCODE) )  //获取安装硬件信息
	{
	
	}
	else if ( !strcmp("60024", vmsg.TRNCODE) )  //获取安装软件信息
	{
		
	}
	else if ( !strcmp("60025", vmsg.TRNCODE) )  //结束系统进程
	{
		
	}
	else if( !strcmp("60099", vmsg.TRNCODE) )		//强制清除命令
	{
	}
...
{
	if(nIDEvent == STATTIMER_EVENT)
	{
		TimerSend_Stat();
	}
	else if(nIDEvent == RTCARDTIMER_EVENT)
	{
		TimerSend_RtCard();
	}
	else if(nIDEvent == TRANTIMER_EVENT)
	{
		TimerSend_Tran();
	}
...
/*	if(strcmp(RefreshTime_now,RefreshTime))
	...
	{
		memset(szValue,0,sizeof(szValue));
		GetNTReg("SOFTWARE\\Ebring\\Agent\\DevStat","RESP",szValue);
		if(!strcmp(szValue,"0"))
		{
			strcpy(RptStaTime,nowtime);
			return true;
...
void NTLogic::TimerSend_Stat()
{
	...
	if(!PackStat(SendMsg,&SendMsgLen))
	{
...
void NTLogic::TimerSend_Tran()
{
	...
	while(PackTRN(pos,SendMsg,&SendMsgLen))
	{
		...
		m_trace->WTrace(LOG_GRP, LOG_STEP, LT_INFO, "Tran Send Successed");
		char tmpbuf[_MAX_PATH];
		memset(tmpbuf,0x00,sizeof(tmpbuf));
		sprintf(tmpbuf,"SOFTWARE\\EBRING\\Agent\\Config\\FLOWA\\MESSAGES\\TRN\\%d", pos);
